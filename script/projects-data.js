const projects = [
    {
        id: 7,
        type: "career",
        title: "MES 생산 인터페이스 운영 및 데이터 정합성 개선",
        company: "(주)에이시에스 · 제조 MES 운영·개발",
        tags: [
            { name: "경력", color: "orange" },
            { name: "MES", color: "orange" },
            { name: "SQL Server", color: "orange" },
            { name: "데이터 정합성", color: "orange" }
        ],
        description: `
            제조 현장의 MES(Manufacturing Execution System) 운영을 지원하며,
            생산지시·LOT·공정 인수인계·불량 처리 데이터의 연계와 인터페이스를 관리한 실무 경력입니다.
            ERP·설비와 MES 사이를 오가는 생산 데이터가 정확하게 처리되도록 운영하는 역할을 담당했습니다.
            <br><br>
            <b>업무 접근 방식:</b> 단순 유지보수에 그치지 않고, 운영 중 발생하는 데이터 불일치와 예외 상황을 분석해
            인터페이스 로직 자체를 개선하는 데 집중했습니다. LOT 추적을 위한 DB 프로시저와 데이터 흐름을 직접 추적해
            원인을 규명하고, 기존 연계 구조를 검토한 뒤 신규 로직을 설계해 데이터 누락·연계 오류를 해소했습니다.
            현업 운영 부서와 협업하여 생산 라인의 긴급 이슈에도 대응했습니다.
        `,
        features: [
            "생산 LOT 데이터 조회 및 인터페이스 프로시저 분석 (LOT 추적용 DB 프로시저·데이터 흐름 확인, 생산 이력 검증)",
            "공정 인수·인계 데이터 정합성 개선 (중복 발생 원인 분석, 인수·인계 시점 기준 처리 로직 검토·개선, 매핑 오류·예외 케이스 대응)",
            "불량 처리 프로세스 지원 (불량취소·불량 유형 데이터 검증, 공정별 불량 이력 확인 및 운영 이슈 대응)",
            "MES 인터페이스 로직 개선 (생산지시·작업지시·LOT 연계 로직 분석, 기존 구조 검토 후 신규 로직 설계, 데이터 누락·연계 오류 수정)",
            "현장 운영 이슈 대응 (MES-설비 간 데이터 송수신 문제 분석, 작업지시·생산 이력 조회로 장애 원인 파악, 운영 부서 협업 긴급 대응)"
        ],
        techStack: [
            "Backend C#",
            ...
            "Backend SQL 데이터 처리 로직",
            "Tools MES",
            "Tools ERP / 설비 인터페이스"
        ],
        role: {
            team: "제조 MES 운영·개발 (실무 경력)",
            mine: [
                "생산 인터페이스(생산지시·LOT·공정·불량) 운영 및 데이터 연계 관리",
                "DB 프로시저 분석을 통한 생산 데이터 검증 및 장애 원인 규명",
                "공정 인수·인계 데이터 처리 로직 개선 및 예외 케이스 대응",
                "MES-ERP/설비 간 인터페이스 오류 분석 및 신규 연계 로직 설계"
            ]
        },
        troubleShooting: [
            {
                problem: "공정 인수·인계 시 동일 데이터가 중복 처리되어 생산 이력 정합성 저하",
                solution: "인수·인계 시점을 기준으로 데이터 처리 로직을 재검토하고 매핑 기준을 정비, 예외 케이스 분기 처리 추가",
                result: "공정 간 데이터 중복 해소 및 생산 이력 정합성 확보"
            },
            {
                problem: "MES와 설비/ERP 간 데이터 송수신 누락·연계 오류로 작업지시·생산 이력 불일치 발생",
                solution: "DB 프로시저와 인터페이스 데이터 흐름을 추적해 누락 구간을 식별하고 연계 로직 수정, 운영 부서와 협업해 긴급 대응",
                result: "데이터 누락·연계 오류 해소로 운영 안정성 향상"
            }
        ],
        results: [
            "운영 중 발생한 데이터 불일치·예외 상황 분석으로 인터페이스 정합성 개선",
            "DB 프로시저·데이터 흐름 직접 추적을 통한 장애 원인 규명 역량 확보",
            "현업 운영 부서와 협업한 생산 라인 긴급 이슈 대응 경험"
        ],
        duration: "2026.03 ~ 재직 중"
    },
    {
        id: 1,
